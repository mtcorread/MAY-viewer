import { useEffect, useRef, useState } from "react";
import maplibregl from "maplibre-gl";
import { Protocol, PMTiles } from "pmtiles";
import { useStore } from "../state/store";
import { CACHE, type BoundaryLevel } from "../data/manifest";
import { compact } from "../util/format";

// Offline-first map. Two render modes, both fed by bounded PMTiles tile
// requests — no external basemap, no whole-file fetch (non-negotiable #1):
//
//  • "boundaries" — real geo polygons baked by prep, one source-layer per
//    geo level. Each level is a separate vector source pinned to its single
//    bake zoom; MapLibre overzooms that one zoom across the level's
//    contiguous band, so only viewport tiles are ever fetched and zooming
//    in walks coarse→fine level by level. Polygons are the click target →
//    store.drillToGeo (this is what finally gives the map a geo_id).
//  • "hexbin" — the original density backdrop, kept as the no-shapes
//    fallback (non-negotiable #6) and a toggle when shapes do exist.
//
// deck.gl is intentionally not used: native GPU fill/line is lighter and
// fully offline.

const RAMP: Record<string, string[]> = {
  // perceptual low→high; first stop is the dark ground
  _people: ["#0f2530", "#13525a", "#1f8f7e", "#73d39b", "#e9f7b5"],
  _venues: ["#241a10", "#5a3a16", "#9c6321", "#e0962f", "#ffd27a"],
};

function rampFor(layer: string): string[] {
  return RAMP["_" + layer] ?? RAMP._people;
}

const ACCENT = "#5fe3c0";
const NONE_FILTER: maplibregl.FilterSpecification = ["==", ["get", "geo_id"], -1];

export function MapView() {
  const ref = useRef<HTMLDivElement>(null);
  const mapRef = useRef<maplibregl.Map | null>(null);
  const {
    manifest,
    mapMode,
    setMapMode,
    hexLayer,
    setHexLayer,
    selected,
    basemap,
    basemapOn,
    setBasemapOn,
  } = useStore();
  const layers = manifest?.artifacts.hexbin.layers ?? [];
  const boundaries = manifest?.artifacts.boundaries;
  const [domain, setDomain] = useState<[number, number]>([1, 1000]);

  // Build the map once.
  useEffect(() => {
    if (!ref.current || mapRef.current || !manifest) return;
    const proto = new Protocol();
    maplibregl.addProtocol("pmtiles", proto.tile);

    const hexUrl = `${CACHE}/${manifest.artifacts.hexbin.path}`;
    const hexAbs = new URL(hexUrl, location.href).href;
    const zooms = [...manifest.artifacts.hexbin.zooms].sort((a, b) => a - b);
    // The hexbin pyramid is SPARSE (tiles only at discrete RES_TO_ZOOM
    // zooms): pin the source to one representative zoom and overzoom it.
    const srcZoom = zooms[Math.min(1, zooms.length - 1)] ?? 0;

    const sources: Record<string, maplibregl.SourceSpecification> = {
      hex: {
        type: "vector",
        tiles: [`pmtiles://${hexAbs}/{z}/{x}/{y}`],
        minzoom: srcZoom,
        maxzoom: srcZoom,
      },
    };

    // Opt-in online basemap (drawn beneath everything). The source/layer
    // exist only when the operator enabled it; visibility tracks the toggle
    // so no tiles are fetched until it is actually on.
    const bm = useStore.getState().basemap;
    const bmOn = useStore.getState().basemapOn;
    const styleLayers: maplibregl.LayerSpecification[] = [
      { id: "bg", type: "background", paint: { "background-color": "#070809" } },
    ];
    if (bm) {
      sources.basemap = {
        type: "raster",
        tiles: bm.tiles,
        tileSize: bm.tileSize,
        maxzoom: bm.maxzoom,
        attribution: bm.attribution,
      };
      styleLayers.push({
        id: "basemap",
        type: "raster",
        source: "basemap",
        layout: { visibility: bmOn ? "visible" : "none" },
        paint: { "raster-opacity": 0.85 },
      });
    }
    styleLayers.push(
      ...manifest.artifacts.hexbin.layers.map(
        (lyr): maplibregl.LayerSpecification => ({
          id: `hex-${lyr}`,
          type: "fill",
          source: "hex",
          "source-layer": lyr,
          layout: { visibility: "none" },
          paint: {
            "fill-color": "#1f8f7e",
            "fill-opacity": 0.82,
            "fill-outline-color": "rgba(255,255,255,0.06)",
          },
        }),
      ),
    );

    // One vector source + fill/line/highlight layers per boundary level.
    let bndAbs = "";
    if (boundaries) {
      bndAbs = new URL(`${CACHE}/${boundaries.path}`, location.href).href;
      for (const lv of boundaries.levels) {
        const sid = `bnd-${lv.level_name}`;
        // Pin to the single baked zoom: MapLibre overzooms it across the
        // level's whole band, fetching only viewport tiles.
        sources[sid] = {
          type: "vector",
          tiles: [`pmtiles://${bndAbs}/{z}/{x}/{y}`],
          minzoom: lv.bake_zoom,
          maxzoom: lv.bake_zoom,
        };
        // MapLibre hides a layer at z >= maxzoom, so extend by 1 (cap 24)
        // to keep the band's top zoom inclusive.
        const lmax = Math.min(24, lv.maxzoom + 1);
        styleLayers.push(
          {
            id: `bfill-${lv.level_name}`,
            type: "fill",
            source: sid,
            "source-layer": lv.level_name,
            minzoom: lv.minzoom,
            maxzoom: lmax,
            layout: { visibility: "none" },
            paint: { "fill-color": ACCENT, "fill-opacity": 0.08 },
          },
          {
            id: `bline-${lv.level_name}`,
            type: "line",
            source: sid,
            "source-layer": lv.level_name,
            minzoom: lv.minzoom,
            maxzoom: lmax,
            layout: { visibility: "none", "line-join": "round" },
            paint: {
              "line-color": ACCENT,
              "line-opacity": 0.9,
              "line-width": ["interpolate", ["linear"], ["zoom"], 3, 1.1, 9, 1.6, 18, 2.4],
            },
          },
          {
            id: `bsel-${lv.level_name}`,
            type: "fill",
            source: sid,
            "source-layer": lv.level_name,
            minzoom: lv.minzoom,
            maxzoom: lmax,
            filter: NONE_FILTER,
            layout: { visibility: "none" },
            paint: { "fill-color": ACCENT, "fill-opacity": 0.22 },
          },
          {
            id: `bselline-${lv.level_name}`,
            type: "line",
            source: sid,
            "source-layer": lv.level_name,
            minzoom: lv.minzoom,
            maxzoom: lmax,
            filter: NONE_FILTER,
            layout: { visibility: "none", "line-join": "round" },
            paint: { "line-color": "#bdfff0", "line-width": 2.4 },
          },
        );
      }
    }

    const startMode = useStore.getState().mapMode;
    const bndMinZoom = boundaries
      ? Math.min(...boundaries.levels.map((l) => l.bake_zoom))
      : 0;
    const useBnd = startMode === "boundaries" && !!boundaries;

    const map = new maplibregl.Map({
      container: ref.current,
      // No `glyphs` key: we render no symbol/text layers, and MapLibre's
      // style validator now rejects `glyphs: undefined` (aborting the whole
      // style load → blank map). Omit it entirely.
      style: { version: 8, sources, layers: styleLayers },
      minZoom: useBnd ? bndMinZoom : srcZoom,
      maxZoom: useBnd ? 22 : Math.max(...zooms) + 4,
      attributionControl: false,
      dragRotate: false,
    });
    map.addControl(new maplibregl.NavigationControl({ showCompass: false }), "top-right");
    mapRef.current = map;

    // Fit to the active archive's own bounds (header read only — bounded).
    // If a unit is already selected the `selected` effect re-fits to it.
    const fitArchive = (url: string, max: number) =>
      void new PMTiles(url).getHeader().then((h) => {
        map.fitBounds(
          [
            [h.minLon, h.minLat],
            [h.maxLon, h.maxLat],
          ],
          { padding: 50, animate: false, maxZoom: max },
        );
      });
    if (useBnd) fitArchive(`${CACHE}/${boundaries!.path}`, 9);
    else fitArchive(hexUrl, Math.max(...zooms));

    const recolor = () => {
      if (useStore.getState().mapMode !== "hexbin") return;
      const lyr = useStore.getState().hexLayer;
      const feats = map.queryRenderedFeatures({ layers: [`hex-${lyr}`] });
      let max = 0;
      for (const f of feats) {
        const c = Number(f.properties?.count ?? 0);
        if (c > max) max = c;
      }
      if (max <= 0) return;
      setDomain([1, max]);
      const ramp = rampFor(lyr);
      const stops: (number | string)[] = [];
      ramp.forEach((col, i) => {
        const t = i / (ramp.length - 1);
        const v = Math.max(1, Math.round(Math.pow(max, t)));
        stops.push(v, col);
      });
      map.setPaintProperty(`hex-${lyr}`, "fill-color", [
        "interpolate",
        ["linear"],
        ["to-number", ["get", "count"], 0],
        ...stops,
      ]);
    };
    map.on("idle", recolor);

    // Boundary click → drill. queryRenderedFeatures only returns the level
    // visible at the current zoom band, so this resolves to that level.
    if (boundaries) {
      const allFillIds = boundaries.levels.map((l) => `bfill-${l.level_name}`);
      // Only query layers that actually exist yet (style may still be
      // loading, or a layer outside the current zoom band is absent).
      const liveFillIds = () =>
        map.style && map.isStyleLoaded()
          ? allFillIds.filter((id) => map.getLayer(id))
          : [];
      map.on("click", (e) => {
        if (useStore.getState().mapMode !== "boundaries") return;
        const ids = liveFillIds();
        if (!ids.length) return;
        const hit = map.queryRenderedFeatures(e.point, { layers: ids })[0];
        if (!hit) return;
        const p = hit.properties ?? {};
        const gid = Number(p.geo_id);
        const lvl = Number(p.level);
        if (Number.isFinite(gid) && Number.isFinite(lvl))
          void useStore.getState().drillToGeo(gid, lvl);
      });
      map.on("mousemove", (e) => {
        if (useStore.getState().mapMode !== "boundaries") return;
        const ids = liveFillIds();
        const over =
          ids.length > 0 &&
          map.queryRenderedFeatures(e.point, { layers: ids }).length > 0;
        map.getCanvas().style.cursor = over ? "pointer" : "";
      });
    }

    return () => {
      map.remove();
      mapRef.current = null;
      maplibregl.removeProtocol("pmtiles");
    };
  }, [manifest, boundaries]);

  // Apply the active render mode: layer visibility + zoom constraints.
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
    const apply = () => {
      const bnd = mapMode === "boundaries" && !!boundaries;
      for (const l of layers)
        if (map.getLayer(`hex-${l}`))
          map.setLayoutProperty(
            `hex-${l}`,
            "visibility",
            !bnd && l === hexLayer ? "visible" : "none",
          );
      if (boundaries) {
        for (const lv of boundaries.levels)
          for (const pfx of ["bfill", "bline", "bsel", "bselline"])
            if (map.getLayer(`${pfx}-${lv.level_name}`))
              map.setLayoutProperty(
                `${pfx}-${lv.level_name}`,
                "visibility",
                bnd ? "visible" : "none",
              );
        const zs = manifest!.artifacts.hexbin.zooms;
        if (bnd) {
          map.setMinZoom(Math.min(...boundaries.levels.map((l) => l.bake_zoom)));
          map.setMaxZoom(22);
        } else {
          const sorted = [...zs].sort((a, b) => a - b);
          map.setMinZoom(sorted[Math.min(1, sorted.length - 1)] ?? 0);
          map.setMaxZoom(Math.max(...zs) + 4);
        }
      }
    };
    if (map.isStyleLoaded()) apply();
    else map.once("styledata", apply);
  }, [mapMode, hexLayer, layers, boundaries, manifest]);

  // Basemap on/off (only when the operator opted in). No tiles are
  // requested until the layer becomes visible.
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !basemap) return;
    const apply = () => {
      if (map.getLayer("basemap"))
        map.setLayoutProperty(
          "basemap",
          "visibility",
          basemapOn ? "visible" : "none",
        );
    };
    if (map.isStyleLoaded()) apply();
    else map.once("styledata", apply);
  }, [basemap, basemapOn]);

  // Selected unit (from tree OR map click) → highlight + fit to its extent.
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !boundaries) return;
    let alive = true;
    const { selected, ensureLevel } = useStore.getState();
    if (!selected) return;
    const bandByLevel = new Map<number, BoundaryLevel>(
      boundaries.levels.map((l) => [l.level, l]),
    );

    void ensureLevel(selected.level).then(() => {
      if (!alive) return;
      const sel = useStore.getState().selected;
      if (!sel) return;

      const setSel = () => {
        for (const lv of boundaries.levels) {
          const f: maplibregl.FilterSpecification =
            lv.level === sel.level
              ? ["==", ["get", "geo_id"], sel.geo_id]
              : NONE_FILTER;
          if (map.getLayer(`bsel-${lv.level_name}`))
            map.setFilter(`bsel-${lv.level_name}`, f);
          if (map.getLayer(`bselline-${lv.level_name}`))
            map.setFilter(`bselline-${lv.level_name}`, f);
        }
      };
      if (map.isStyleLoaded()) setSel();
      else map.once("styledata", setSel);

      if (useStore.getState().mapMode !== "boundaries") return;
      const r = useStore.getState().aggRow(sel.level, sel.geo_id);
      const n = (v: unknown) => (typeof v === "number" && isFinite(v) ? v : null);
      const mnx = r && n(r["geo:bbox_minlon"]);
      const mny = r && n(r["geo:bbox_minlat"]);
      const mxx = r && n(r["geo:bbox_maxlon"]);
      const mxy = r && n(r["geo:bbox_maxlat"]);
      // Keep the selected level itself visible: don't zoom past its band.
      const band = bandByLevel.get(sel.level);
      const fitMax = Math.min(17, band ? band.maxzoom : 17);
      if (mnx != null && mny != null && mxx != null && mxy != null) {
        map.fitBounds(
          [
            [mnx, mny],
            [mxx, mxy],
          ],
          { padding: 60, maxZoom: fitMax, duration: 600 },
        );
      } else {
        const lon = r && n(r["geo:lon"]);
        const lat = r && n(r["geo:lat"]);
        if (lon != null && lat != null)
          map.easeTo({ center: [lon, lat], zoom: fitMax, duration: 600 });
      }
    });
    return () => {
      alive = false;
    };
  }, [boundaries, selected, mapMode]);

  const ramp = rampFor(hexLayer);
  const inBnd = mapMode === "boundaries" && !!boundaries;

  return (
    <div className="maproot">
      <div ref={ref} style={{ position: "absolute", inset: 0 }} />

      <div className="map-hint">
        {inBnd
          ? "click a region to drill in · zoom to walk levels"
          : "density backdrop · navigate the geography in the left rail"}
      </div>

      <div className="map-overlay">
        {inBnd ? (
          <div className="card legend">
            <div className="lh">
              <span className="t">Geography</span>
              <span className="eyebrow">{boundaries!.levels.length} levels</span>
            </div>
            <div className="lv-bands">
              {boundaries!.levels.map((l) => (
                <div key={l.level_name} className="lv-band">
                  <span className="nm mono">{l.level_name}</span>
                  <span className="zr">
                    z{l.minzoom}–{l.maxzoom}
                  </span>
                </div>
              ))}
            </div>
          </div>
        ) : (
          <div className="card legend">
            <div className="lh">
              <span className="t">Hexbin density</span>
              <span className="eyebrow">{hexLayer}</span>
            </div>
            <div
              className="ramp"
              style={{ background: `linear-gradient(90deg, ${ramp.join(",")})` }}
            />
            <div className="scale">
              <span>{compact(domain[0])}</span>
              <span>per&nbsp;cell</span>
              <span>{compact(domain[1])}</span>
            </div>
          </div>
        )}

        {boundaries && (
          <div className="card layer-switch">
            <button className={inBnd ? "on" : ""} onClick={() => setMapMode("boundaries")}>
              Boundaries
            </button>
            <button className={!inBnd ? "on" : ""} onClick={() => setMapMode("hexbin")}>
              Hexbin
            </button>
          </div>
        )}

        {!inBnd && layers.length > 1 && (
          <div className="card layer-switch">
            {layers.map((l) => (
              <button
                key={l}
                className={l === hexLayer ? "on" : ""}
                onClick={() => setHexLayer(l)}
              >
                {l}
              </button>
            ))}
          </div>
        )}

        {basemap && (
          <div className="card layer-switch">
            <button
              className={!basemapOn ? "on" : ""}
              onClick={() => setBasemapOn(false)}
            >
              No basemap
            </button>
            <button
              className={basemapOn ? "on" : ""}
              onClick={() => setBasemapOn(true)}
            >
              {basemap.name}
            </button>
          </div>
        )}
      </div>

      {basemap && basemapOn && basemap.attribution && (
        <div className="map-attrib">{basemap.attribution}</div>
      )}
    </div>
  );
}
